...
(function($){
    $.fn.teninedialog = function(options){
        ...
        return this.each(function(){
            var obj=$(this);
            modalID=getModalID();
            var tmpHtml='<div id="{ID}" class="modal fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header">{times}<h5 id="myModalLabel">{title}</h5></div><div class="modal-body"><div class="{bodyStyle}"><h5>{bodyTitle}</h5><p>{body}</p></div></div><div class="modal-footer">{button}</div></div></div></div>';
            var timesHtml = '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>';
            if (!options.showTimesButton) {timesHtml='';}
            ...
            //替换模板标记
            tmpHtml=tmpHtml.replace(/{ID}/g,modalID).replace(/{times}/g,timesHtml).replace(/{title}/g,options.title).replace(/{bodyTitle}/g,options.contentTitle).replace(/{bodyStyle}/g,options.contentStyle).replace(/{body}/g,options.content).replace(/{button}/g,buttonHtml);
            obj.append(tmpHtml);
